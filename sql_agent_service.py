...
class SQLAgentService:
    ...
    def _load_place_categories(self):
        return {
            'regions': [
    ...
    def _preload_schemas(self):
        print("🔄 Preloading schemas for performance...")
        for mv in self.materialized_views:
            try:
                self._schema_cache[mv] = self.get_table_schema(mv)
                print(f"✅ Cached schema for {mv}")
            except Exception as e:
                print(f"⚠️ Failed to cache schema for {mv}: {e}")
        
        if self.main_table in self._schema_cache:
            schema_info = self._schema_cache[self.main_table]
            self._column_names_cache = self._extract_column_names(schema_info)
            print(f"✅ Cached column names: {len(self._column_names_cache)} columns")
    
    def _extract_column_names(self, schema_info: str) -> List[str]:
        column_names = []
...
OUTPUT ONLY THE SQL - NO OTHER TEXT:"""
    ...
    async def query(self, question: str, chart_type: str = "auto") -> QueryResponse:
        ...
        try:
            ...
            # Use cached schema instead of fetching on every request
            schema_info = self._schema_cache.get(self.main_table, self.get_table_schema(self.main_table))
            column_names = self._column_names_cache or self._extract_column_names(schema_info)
            
            columns_list = ', '.join(column_names) if column_names else "Unable to extract columns"
            
            performance_log['schema_extraction'] = time.time() - start_time
            ...
            # Optimized place extraction using cached data
            place_extraction_start = time.time()
            found_places = self._extract_places_from_question_optimized(question)
            performance_log['place_extraction'] = time.time() - place_extraction_start
...
OUTPUT ONLY THE SQL - NO OTHER TEXT:"""
            ...
            performance_log['prompt_generation'] = time.time() - prompt_start
            
            # LLM call for SQL generation
            llm_start = time.time()
            sql_response = self.llm.invoke(prompt)
            raw_sql = sql_response.content.strip()
            performance_log['llm_sql_generation'] = time.time() - llm_start
            
            print(f"Debug - Raw LLM response: '{raw_sql}'")
            
            # SQL cleaning
            sql_cleaning_start = time.time()
            ...
            # Optimized city name corrections
            ...
            performance_log['sql_cleaning'] = time.time() - sql_cleaning_start
            ...
            performance_log['sql_validation'] = time.time() - validation_start
            ...
            try:
                ...
                performance_log['database_execution'] = time.time() - db_start
    ...
    def _extract_places_from_question_optimized(self, question_text):
        found_places = []
        for category, places in self._place_categories.items():
            for place in places:
    ...
    async def _generate_insights_async(self, df: pd.DataFrame, chart_type: str, question: str) -> str:
        try:
            if df.empty:
                question_lower = question.lower()
...
Keep the response concise (2-3 sentences) and focus on actionable insights. Use only Arabic . Provide meaningful business intelligence rather than just data description."""
    ...
    async def _generate_related_analysis_async(self, df: pd.DataFrame, question: str, schema_info: str) -> List[str]:
        try:
